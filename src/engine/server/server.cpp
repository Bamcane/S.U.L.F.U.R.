...
void CServer::GenerateServerInfo(CPacker *pPacker, bool IncludeClientInfo)
{
	...
	pPacker->AddInt(Config()->m_SvMaxClients); // max players
...
int CServer::LoadMap(const char *pMapName, bool Generated)
{
	char aBuf[IO_MAX_PATH_LENGTH];
	str_format(aBuf, sizeof(aBuf), Generated ? "generatedmaps/%s.map" : "maps/%s.map", pMapName);
		...
		while(m_RunServer)
		{
			...
			if(m_MapReload || m_CurrentGameTick >= 0x6FFFFFFF) //	force reload to make sure the ticks stay within a valid range
			{
				...
				if(LoadMap(Config()->m_SvMap))
				{
					...
					for(int c = 0; c < MAX_CLIENTS; c++)
					{
						m_aClients[c].m_MapID = m_BaseMapUuid;
...
void CServer::RequestNewWorld(int ClientID, const char *pWorldName)
{
	...
	std::thread([this](int ClientID, string WorldName)
	{
		...
		if(!m_uMapDatas.count(MapUuid) && !LoadMap(WorldName, true))
		{
			CMapGen *pMapGen = new CMapGen(Storage(), Console());
			pMapGen->CreateMap(WorldName, true);
			LoadMap(WorldName, true);
		}
